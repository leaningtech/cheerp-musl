...
cd build_genericjs
CHEERP_PREFIX=/opt/cheerp RANLIB="${CHEERP_PREFIX}/bin/llvm-ar s" AR="${CHEERP_PREFIX}/bin/llvm-ar" CC="${CHEERP_PREFIX}/bin/clang -target cheerp" LD="${CHEERP_PREFIX}/bin/llvm-link" CPPFLAGS="-isystem ${CHEERP_PREFIX}/lib/clang/16/include" CFLAGS="-Wno-int-conversion" ../configure --target=cheerp --disable-shared --prefix=${CHEERP_PREFIX}
cd ..

mkdir -p build_wasm
cd build_wasm
CHEERP_PREFIX=/opt/cheerp RANLIB="${CHEERP_PREFIX}/bin/llvm-ar s" AR="${CHEERP_PREFIX}/bin/llvm-ar" CC="${CHEERP_PREFIX}/bin/clang -target cheerp-wasm" LD="${CHEERP_PREFIX}/bin/llvm-link" CPPFLAGS="-isystem ${CHEERP_PREFIX}/lib/clang/16/include" CFLAGS="-Wno-int-conversion" ../configure --target=cheerp-wasm --disable-shared --prefix=${CHEERP_PREFIX}
